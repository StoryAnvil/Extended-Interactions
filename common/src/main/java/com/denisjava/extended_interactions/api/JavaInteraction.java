...
public abstract class JavaInteraction extends ExtInteraction {
    ...
    @Override
    public final void handleExecution(Player player, MenuTarget target, String argumentId) {
        if (target instanceof MenuTarget.BlockTarget block) {
            handleBlockExecution(player, block.getLevel(), block.getPos(), block.getLevel().getBlockState(block.getPos()), argumentId);
        } else if (target instanceof MenuTarget.EntityTarget entity) {
            handleEntityExecution(player, player.level(), player.level().getEntity(entity.getEntityId()), argumentId);
        }
    }
