...
public class MaliLibAction implements DataDrivenAction {
    public static final MapCodec<MaliLibAction> CODEC = RecordCodecBuilder.mapCodec(inst -> inst.group(
            Codec.STRING.fieldOf("category").forGetter(MaliLibAction::getCategory),
            Codec.STRING.fieldOf("keybind").forGetter(MaliLibAction::getKeybind)
    ).apply(inst, MaliLibAction::new));

    public final String category;
    public final String keybind;
    private IHotkey cachedKey = null;

    public MaliLibAction(String category, String keybind) {
